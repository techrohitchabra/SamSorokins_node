...
// In-memory cache for raw computed resolved duplicate groups
let resolvedRawCache = null;
let resolvedCacheTime = 0;
const CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes TTL

export const clearResolvedDuplicatesCache = () => {
  resolvedRawCache = null;
  resolvedCacheTime = 0;
};

const computeDuplicateGroupsFromDB = async (targetFormId = null) => {
  const matchStage = { isDuplicateResolved: true };
  if (targetFormId) {
...
export default async (req, res, next) => {
  try {
    ...
    // Compute or use cached raw resolved duplicate groups
    let activeRawGroups;
    if (targetFormId) {
      activeRawGroups = await computeDuplicateGroupsFromDB(targetFormId);
    } else {
      if (!resolvedRawCache || now - resolvedCacheTime > CACHE_TTL_MS) {
        resolvedRawCache = await computeDuplicateGroupsFromDB();
        resolvedCacheTime = now;
      }
      activeRawGroups = resolvedRawCache || [];
    }
    ...
    console.error("[submissionsData/getResolvedDuplicates]", error);
    next(error);
  }
};
